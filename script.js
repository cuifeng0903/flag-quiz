...
function createConfetti() {
  ...
  for (let i = 0; i < 300; i++) {
    const angle = (Math.random() - 0.5) * Math.PI / 2; // 角度拡大
    const speed = Math.random() * 4 + 6; // 速度遅く
    particles.push({
...
function prepareQuiz() {
  ...
  const pool = FLAGS.filter(f => !selected.includes(f));
...
function newQuestion() {
  ...
  document.getElementById("flag-area").innerHTML = `<img src="${currentFlag.file}" alt="" loading="eager">`;
...
function selectAnswer(isCorrect, btn, choices) {
  ...
  if (isCorrect) {
    ...
    // 誤答記憶
    if (!missedFlags.find(f => f.code === currentFlag.code)) {
...
function startReview(flag) {
  ...
  document.getElementById("review-flag-area").innerHTML = `<img src="${flag.file}" alt="" loading="eager">`;
